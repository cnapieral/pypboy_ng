    ...
    def redraw_map(self, coef=1):
        ...
        for tag in self._mapper.transpose_tags((self._size / coef, self._size / coef), (self._size / 2, self._size / 2)):
            if tag[3] in config.AMENITIES:
                ...
                scaled_image = pygame.transform.scale(image, (10, 10))
                self._map_surface.blit(scaled_image, (tag[1], tag[2]))
...
class MapSquare(game.Entity):
    def __init__(self, size, map_position, parent, *args, **kwargs):
        ...
        self._size = size
        self.parent = parent
        self._map_surface = pygame.Surface((size, size))
        ...
        for x in range(-4, 4):
            for y in range(-2, 2):
                square = MapSquare(
                    86,
                    (
                        self._starting_position[0] + (delta * x),
                        self._starting_position[1] - (delta * y)
                    ),
                    self
                )
                square.fetch_map()
    ...
    def draw_tags(self):
        ...
        for name in self.tags:
            if self.tags[name][2] in config.AMENITIES:
                ...
                scaled_image = pygame.transform.scale(image, (10, 10))
                self.image.blit(scaled_image, (self.tags[name][0], self.tags[name][1]))
